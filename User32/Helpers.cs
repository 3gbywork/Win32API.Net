...
namespace WinApi.Net.User32
{
    public static class User32Helpers
    {
            ...
            out WindowClassEx classInfo)
        {
            ...
            if (User32Methods.GetClassInfoEx(hInstance, lpClassName, out classExBlittable))
            {
                classInfo = new WindowClassEx
                {
                    ...
                    WindowProc = MarshalEx.GetDelegateForFunctionPointer<WindowProc>(classExBlittable.WindowProc),
        ...
        public static uint SendInput(Input[] inputs)
        {
            ...
            var size = MarshalEx.SizeOf<Input>();
        ...
        {
            fixed (Input* ptr = &input) return User32Methods.SendInput(1, new IntPtr(ptr), MarshalEx.SizeOf<Input>());
        }

        public static unsafe bool GetTitleBarInfo(IntPtr hwnd, ref TitleBarInfo pti)
        {
            if (pti.Size == 0) pti.Size = (uint)MarshalEx.SizeOf<TitleBarInfo>();
